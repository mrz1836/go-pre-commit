...
func TestIsGoFile(t *testing.T) {
	...
	tests := []struct {
		...
		{"Go source file", testFileMainGo, true},
...
func TestDetectLanguage(t *testing.T) {
	...
	tests := []struct {
		...
		// Shell scripts
		{"Shell", "script.sh", fileTypeShell},
		{"Bash", "script.bash", fileTypeShell},
		{"Zsh", "script.zsh", fileTypeShell},
		{"Fish", "script.fish", fileTypeShell},
		{"PowerShell", "script.ps1", "powershell"},

		// Data formats
		{"SQL", "query.sql", "sql"},
		{"Markdown", testFileReadme, "markdown"},
		{"Text", "notes.txt", "text"},
		...
		// Unknown
		{"Unknown extension", "file.xyz", fileTypeUnknown},
		{"No extension", "README", fileTypeUnknown},
		{"Binary", "app.exe", fileTypeUnknown},
	}
...
func TestIsGeneratedFile(t *testing.T) {
	...
	tests := []struct {
		...
		// Non-generated files
		{"Regular Go file", testFileMainGo, "package main\n\nfunc main() {}", false},
...
func TestIsExcludedPath(t *testing.T) {
	tests := []struct {
		...
		{"Regular file", testFileMainGo, nil, false},
...
func TestMatchesPattern(t *testing.T) {
	...
	tests := []struct {
		...
		// Exact matches
		{"Exact match", testFileFileGo, testFileFileGo, true},
		{"No match", testFileFileGo, "other.go", false},

		// Directory patterns
		{"Dir pattern start", "vendor/file.go", "vendor/", true},
		{"Dir pattern middle", "src/vendor/file.go", "vendor/", true},
		{"Dir pattern no match", testFileFileGo, "vendor/", false},

		// Simple globs
		{"Star at end", testFileFileGo, testPatternGoStar, true},
		{"Star at end no match", "file.py", testPatternGoStar, false},
		{"Star at start", testFileMainGo, "*main.go", true},
		{"Star at start no match", testFileMainGo, "*test.go", false},
		{"Star in middle", "file_test.go", "file_*.go", true},
		{"Star in middle no match", testFileMainGo, "file_*.go", false},
		...
		{"Extension only", "any.go", testPatternGoStar, true},
...
func TestGlobMatch(t *testing.T) {
	...
	tests := []struct {
		...
		{"Star at end", testFileTestGo, testPatternGoStar, true},
		{"Star at end no match", "test.py", testPatternGoStar, false},
		{"Star at start", testFileFileGo, "*go", true},
		{"Star at start no match", "file.py", "*go", false},
		{"Star middle", "file_test.go", "file*.go", true},

		// Multiple stars
		{"Two stars", "path/file.go", "*/*.go", true},
		{"Two stars no match", testFileFileGo, "*/*.go", false},

		// Edge cases
		{"Empty pattern", "file", "", false},
		{"Empty string", "", testPatternGoStar, false},
...
func TestClassifyFile(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:     "Go source file",
			fileName: testFileMainGo,
			content:  "package main\n\nfunc main() {}\n",
			expected: FileInfo{
				Path:      filepath.Join(tempDir, testFileMainGo),
				IsText:    true,
				IsBinary:  false,
				Language:  "go",
				IsGoFile:  true,
...
func TestFilterGoFiles(t *testing.T) {
	...
	testFiles := []struct {
		...
		{testFileMainGo, "package main", true, false, false},
...
func TestFilterTextFiles(t *testing.T) {
	...
	testFiles := []struct {
		...
		{testFileMainGo, []byte("package main"), true, false},
...
func TestGetFileStats(t *testing.T) {
	...
	testFiles := []struct {
		...
		{testFileMainGo, []byte("package main"), "go"},
		...
		{testFileReadme, []byte("# Project"), "markdown"},
		{"binary.exe", []byte{0x00, 0xFF}, fileTypeUnknown},
		{".DS_Store", []byte("meta"), fileTypeUnknown},
		{"cache.tmp", []byte("temp"), fileTypeUnknown},
	}
...
func main() {}`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			filePath := filepath.Join(tempDir, testFileTestGo)
			err := os.WriteFile(filePath, []byte(tt.content), 0o600)
			require.NoError(t, err)
...
func TestFileClassifierIntegration(t *testing.T) {
	...
	projectFiles := map[string]string{
		...
		"go.sum":                                 "// go.sum file",
		testFileReadme:                           "# Example Project",
	...
	t.Run("Filter Go files for linting", func(t *testing.T) {
		...
		expectedFiles := []string{
			filepath.Join(tempDir, testFileMainGo),
